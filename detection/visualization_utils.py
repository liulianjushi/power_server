...
def _visualize_boxes_and_masks_and_keypoints(
        image, boxes, classes, scores, masks, keypoints, category_index,
        **kwargs):
    return visualize_boxes_and_labels_on_image_array(
        image,
        boxes,
        ...
        if scores is None or scores[i] > min_score_thresh:
            if classes[i] in category_index.keys():
                ...
                if scores is None:
                    ...
                    if not skip_labels and not skip_scores:
                        display = {}
                        display["class_name"] = category_index[classes[i]]["name"]
                        display["scores"] = 100 * scores[i]
                        display["class"] = classes[i]
                        display_str = '{}: {}%'.format(display["class_name"],
                                                               int(100 * scores[i]))
